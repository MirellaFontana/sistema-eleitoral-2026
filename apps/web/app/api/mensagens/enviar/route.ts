...
// Tentativa real de envio por WhatsApp — sem credencial de provedor configurada ainda
// (dependência bloqueante já registrada pro Módulo 3/matriz de alertas; mesmo ponto de
// integração serve os dois quando o usuário fornecer a credencial).
async function tentarEnviarWhatsApp(_telefone: string, _conteudo: string): Promise<{ ok: boolean; erro?: string }> {
  const configurado = !!process.env.WHATSAPP_API_TOKEN;
  if (!configurado) {
    return { ok: false, erro: "Provedor de WhatsApp ainda não configurado (WHATSAPP_API_TOKEN ausente)." };
  }
  // Quando houver credencial: chamar a API do provedor aqui (Twilio/Meta Cloud API/etc.).
  return { ok: false, erro: "Envio real ainda não implementado — só a configuração de credencial existe." };
}
